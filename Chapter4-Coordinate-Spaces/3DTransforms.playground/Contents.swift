...
// Create Metal Buffer
let originalBuffer = device.makeBuffer(bytes: &verticies, length: MemoryLayout<float3>.stride * verticies.count, options: [])
...
// Draw it - use draw primitives instead of index primites because we're not using submesh indicies
renderEncoder.setVertexBytes(&matrix, length: MemoryLayout<float4x4>.stride, index: 1)
renderEncoder.drawPrimitives(type: .triangle, vertexStart: 0, vertexCount: verticies.count)

let angle = Float.pi / 2.0
var distanceVector = float4(verticies.last!.x,
                            verticies.last!.y,
                            verticies.last!.z, 1)
...
var transformedBuffer = device.makeBuffer(bytes: &verticies, length: MemoryLayout<float3>.stride, options: [])
